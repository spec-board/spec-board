...
/**
 * blind.js - Cross-platform hook dispatcher
 *
 * Blocks access to directories listed in .claude/.claudeignore
 * (defaults: node_modules, __pycache__, venv, .venv, .git, dist, build)
 *
 * Blocking Rules:
 ...
 *
 * Configuration:
 * - Edit .claude/.claudeignore to customize blocked patterns (one per line, # for comments)
 *
 * Platform Detection:
 * - Windows (win32): Uses PowerShell via blind.ps1
 * - Unix (linux/darwin): Uses Bash via blind.sh
...
try {
  ...
  if (platform === 'win32') {
    ...
    const psScript = path.join(scriptDir, 'blind.ps1');
    ...
    // Execute PowerShell script with stdin piped (using execFileSync to prevent shell injection)
    ...
    const bashScript = path.join(scriptDir, 'blind.sh');
    ...
    // Execute bash script with stdin piped (using execFileSync to prevent shell injection)
