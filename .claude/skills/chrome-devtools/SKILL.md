...
Without ImageMagick, screenshots >5MB will not be compressed (may fail to load in Claude).
...
pwd  # Should show current working directory
cd .claude/skills/chrome-devtools/scripts
node screenshot.js --url https://example.com --output ./agent-includes/specs/screenshots/page.png
```
**Important**: Always save screenshots to `./agent-includes/specs/screenshots` directory.

### Automatic Image Compression
Screenshots are **automatically compressed** if they exceed 5MB to ensure compatibility with Claude Code (which has 5MB limit). This uses ImageMagick internally:
...
```bash
node screenshot.js --url https://example.com --output ./agent-includes/specs/screenshots/page.png
ls -lh ./agent-includes/specs/screenshots/page.png  # Verify file exists
